...
kings2 <- read_csv("data/DKMON.csv", na = "NULL"")
...
-  Når man skriver funktionen 'class()' på kings2 i R, aflæser den det som "spec_tbl_df" "tbl_df" "tbl" "data.frame". Dvs. når man kører class funktionen kan vi se at kings2 er en tibble og dermed spytter den ikke udelukkende data frame ud som kings1 gøre. 

3. How many columns does the object have when created with these different functions?
- Ved at køre funktionerne enten head eller glimpse, har mit datasæt 7 kolonner. 

4. Show the dataset so that we can see how R interprets each column
- R ser alle observationerne som "characters", og dermed kan R ikke genkende årstallene som tal, men ser dem som bogstaver hvilket er et problem.
...
```{r}
DKMONduration %>%
  ggplot(aes(x=Regerings_Midt_, y = duration)) + gempoint() + gem_smooth()
```
